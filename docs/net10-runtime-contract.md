...
| P1.28 Legacy managed asset audit closure | 旧 managed 源清点中仅剩两个 active marker 未按原路径 `Link`：`SharedTests/Instructions/Arithmetic/TC_sub.cs` 已由 net10 本地替代版执行并修正 `UIntPtr` subtraction 语义；`AotTests/App.cs` 是旧反射 runner 入口，已由 `LegacyTestRunner` / `RunAotCallInterop` / `RunAotMisc` 取代，真实 AOT 测试体已在 P1.19/P1.20 覆盖 | 旧 managed active marker 扫描仅剩 `AotTests/App.cs :: AotMethod` 与原始 `TC_sub.cs :: UnitTest`；`ManagedNet10.LegacyTests.Program::RunArithmeticSub`、`RunAotCallInterop`、`RunAotMisc`、`RunAll`、默认 `interp-smoke.ps1`、`api-scan.ps1` unsupported `0` 均通过 | 视为 LeanCLR 自有 managed 测试资产清点闭环；不把旧 runner 壳或 net10 替代过的源码副本当作未迁移测试体 |
...
| P1.30 Legacy managed audit gate | 新增 `scripts/dotnet10/legacy-managed-audit.ps1`，展开 `ManagedNet10.LegacyTests.csproj` 的旧 managed `Link` / `LinkBase` / wildcard，扫描未链接旧源码中的未注释 `[UnitTest]`、`Assert`、旧 runner `Main` 和 public static helper；除 `AotTests/App.cs` 旧 runner、`TC_sub.cs` net10 本地替代和 5 个 helper-only fixture 外，出现新活动 marker 即失败 | `powershell -ExecutionPolicy Bypass -File scripts\dotnet10\legacy-managed-audit.ps1` 通过：368 个旧 managed 文件已链接，7 个未链接 active-ish 文件全部已分类 | 作为 LeanCLR 自有 managed 测试资产库存 gate；后续新增旧源码断言或取消注释测试体必须链接进 net10 legacy 项目或显式更新分类 |
| P1.31 Thread-static field instruction coverage | 恢复旧 `TC_ldsfld_interp_threadstatic.cs` / `TC_stsfld_interp_threadstatic.cs` 真测试体，并把 `TypeThreadStaticFields` / `TypeThreadStaticFields2` fixture 纳入 net10 legacy 项目；同时恢复 `TestCaseBase.SetUp/TearDown` 和 `LegacyTestRunner` 每测试方法生命周期调用，保证 thread-static 初始化按旧 runner 语义执行 | `ManagedNet10.LegacyTests.Program::RunFieldThreadStatic` 通过；`RunAll` 通过；`legacy-managed-audit.ps1` 通过，链接旧 managed 文件增至 368 个 | 保留为 `[ThreadStatic]` primitive / enum / object / value type static field load/store regression gate；生命周期钩子作为旧实例测试 runner contract 保留 |
...
| P4.3 LeanCLR self-test gate sweep | 将当前 LeanCLR 自有 gate 固化为解释器默认 smoke、legacy `RunAll`、legacy managed 资产审计、API whitelist、runtime API signature、NKG/Odin workload、host bridge 七场景、P3 managed wrapper 四入口、interpreter smoke matrix，以及 LeanAOT net10 smoke native build/run | `TC_callvir_interp.cs` 保持真实 `[UnitTest]` 测试体并由 `RunFunctionCallvirtInterp` 通过；`legacy-managed-audit.ps1` 通过；`interp-smoke-matrix.ps1 -Configuration Release` 默认全量通过 65 个入口；`aot-smoke.ps1 -Configuration Release -NativeRun` 生成 C++、构建 `aot-tester.exe` 并运行 `ok!` | 作为 Mono 清理后的 LeanCLR 自有测试完成口径；不把 Mono profile、旧 Mono 脚本或整文件注释的虚表测试纳入可接受路径 |
...
- `powershell -ExecutionPolicy Bypass -File scripts\dotnet10\legacy-managed-audit.ps1`：368 个旧 managed 文件已链接，7 个未链接 active-ish 文件全部已分类。
