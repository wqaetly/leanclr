...
if ($IncludeCoreRuntimeAot) {
    $markdown.Add("LeanCLR AOT runs each benchmark through a small AOT-specific host. `StringBuilderBuild`, `ParseAndFormatNumbers`, and `GenericEquality` use checksum-equivalent reduced BCL paths to avoid current CoreLib AOT gaps in `StringBuilder`, culture-aware formatting, and `EqualityComparer<T>`.")
}
